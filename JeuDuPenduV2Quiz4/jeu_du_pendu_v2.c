...
int main(int argc, char* argv[])
{
	...
	/*
	est utilisée pour déclarer un tableau de caractères (une chaîne de caractères)
	qui peut contenir un mot de taille TAILLE_MOT plus un caractère nul ('\0').
	*/
	char motSecret[TAILLE_MOT + 1]; //+ 1 pour le caractère nul
	genererMotSecret(motSecret); // Génération du mot secret
